function Plotz_MAG(z_MAG, zhat_MAG, time)
% Plotz_GPS - Plots GPS measurements vs estimates, ignoring (0,0,0) entries.

figure('Name', 'Magnetometer Measurements', 'NumberTitle', 'off')
...
for i = 1:3
    ...
    valid = any(z_MAG ~= 0, 1);  % True if any axis has a nonzero value

    % Filter data
    t_valid     = time(valid);
    z_valid     = z_MAG(i, valid);
    zhat_valid  = zhat_MAG(i, valid);
